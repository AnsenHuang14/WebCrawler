...
def CrawlPackage(fileindex1=0,fileindex2=99,save_file=False):
	totalapp =0
	for i in xrange(fileindex1,fileindex2+1):
		...
		for j in xrange(len(df)):
			pkg_name = df.loc[j,0]
			for lang in GetLangList(pkg_name):
				totalapp+=1
				FeatureList.append( app_detail(pkg_name,lang))
				print '-------',filename,'-------',totalapp,'-------',(float(j)/len(df))*100,'%'
		if save_file:
			pd.DataFrame(FeatureList).to_json('.\\Data\\'+filename+'.json',orient='records',lines =True)
		return FeatureList

# crawl by pkgname
def CrawlByPkgName(pkgname='com.facebook.katana',save_file=False):
	FeatureList = list()
	for lang in GetLangList(pkgname):
		print lang
		FeatureList.append( app_detail(pkgname,lang))
	if save_file:
		pd.DataFrame(FeatureList).to_json('.\\Data\\'+pkgname+'.json',orient='records',lines =True)
	return FeatureList
...
def MultiThreadCrawlPackage(fileindex1=0,fileindex2=99,n_job=3):
	...
	for i in xrange(n_job):
		...
		if remainder>0 and i <remainder:	
			last_index+=1
		Thread(target=CrawlPackage, name='Thd1',args=(start_index,last_index,True)).start()
		

if __name__ == '__main__':
	# start=datetime.now()
	CrawlByPkgName(save_file=True)
